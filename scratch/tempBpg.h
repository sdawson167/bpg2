...
public:
  ...
  void minimize( FieldProvider &field, TFunctionalCalculator &calculator )
  {
    ...
    while (!stopCriterion)
    {
      ...
      //std::cout << std::endl << "iteration " << m_iterator << std::endl; 

      // optimize field (fixed period)
      optimizeField(field, calculator);
      //std::cout << "after field opt: " << calculator.f(field) << ", iterations " << m_fieldIterator << std::endl;
      
      // optimize period (fixed densities)
      optimizePeriods(field, calculator);
      //std::cout << "after period opt: " << calculator.f(field) << ", iterations " << m_periodIterator << std::endl;
  ...
  void optimizeField( FieldProvider &field, TFunctionalCalculator &calculator )
  {
    ...
    while (!stopCriterion)
    {
      ...
      if (m_fieldIterator % 100 == 0 && tStep >= 0.01) {
	tStep *= 0.1;
	//std::cout << "decreasing tMin, " << tMin << std::endl; 
      }
	...
	cplxFieldData[i][1] = (cplxNestData[i][1] + tStep * laplacian[i] * cplxNlDerivData[i][1]) / A;
      ...
      //std::cout << "iteration " << m_fieldIterator << ", ts: " << tStep << ", error:" << currentError << std::endl << std::endl;;
